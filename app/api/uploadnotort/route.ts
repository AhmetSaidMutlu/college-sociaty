...
function extractSpecificFields(rawText: string) {
  const result: Record<string, string | null> = {
    "Genel Not Ortalaması": null,
  };

  // Find the line containing "Genel Not Ortalaması"
  const gpaLine = rawText.split('\n').find(line => line.includes('Genel Not Ortalaması'));

  if (gpaLine) {
    // Extract the GPA value
    const gpaMatch = gpaLine.match(/([\d.]+)\s*:\s*Genel Not Ortalaması/);
    result["Genel Not Ortalaması"] = gpaMatch ? gpaMatch[1].trim() : null;
  }
